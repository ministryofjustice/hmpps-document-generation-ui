...
```bash
kubectl -n hmpps-document-generation-dev get secret hmpps-document-generation-ui-client-creds -o json | jq -r ".data | map_values(@base64d)"
kubectl -n hmpps-document-generation-dev get secret hmpps-document-generation-ui-auth-code -o json | jq -r ".data | map_values(@base64d)"
```
